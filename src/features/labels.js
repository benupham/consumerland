...
export const labelStyle = function (label, res) {
  ...
  if (!style && view.getResolution() <= labelStyleChange[label.get("type")]) {
    ...
    resolutionCache[label.getId()] >= labelStyleChange[label.get("type")] &&
  ...
  if (isNullOrUndefined(style)) {
    ...
    const backgroundFillColor =
      styleChange === true ? "rgba(255, 255, 255, 0)" : "rgba(255, 255, 255, 0)"
...
export const imageStyle = function (image, res) {
  ...
  if (!style) {
    ...
    const scale = image.get("relativeRadius") > 0.7 ? image.get("relativeRadius") : 0.7 //radius/65 * 2 > 200 ? 1 : radius/65 * 2 / 200;
    if (!icon) {
      icon = new Icon({
        src: image.get("src"),
        size: [200, 200],
